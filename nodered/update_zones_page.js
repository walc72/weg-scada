...
T.push('  <p style="color:#666;font-size:13px;margin:0 0 16px">Configure los rangos Verde/Amarillo/Rojo para cada gauge de cada drive.</p>');
// Group by site
T.push('  <div v-for="site in sites" :key="site" style="margin-bottom:20px">');
T.push('    <div @click="toggleSite(site)" style="display:flex;align-items:center;gap:8px;padding:10px 16px;background:#1a4d8f;color:white;border-radius:8px 8px 0 0;cursor:pointer;user-select:none" :style="{borderRadius:siteOpen[site]?\'8px 8px 0 0\':\'8px\'}">');
T.push('      <span style="font-size:12px;transition:transform 0.2s" :style="{transform:siteOpen[site]?\'rotate(90deg)\':\'rotate(0)\'}">&#9654;</span>');
T.push('      <strong style="font-size:15px">{{ site }}</strong>');
T.push('      <span style="font-size:12px;opacity:0.7">({{ drivesForSite(site).length }} drives)</span>');
T.push('    </div>');
T.push('    <div v-show="siteOpen[site]" style="border:1px solid #e0e0e0;border-top:none;border-radius:0 0 8px 8px;padding:8px">');
// Drive cards (collapsible)
T.push('  <div v-for="(d,i) in drivesForSite(site)" :key="d.name" style="background:white;border:1px solid #e0e0e0;border-radius:8px;margin-bottom:8px;overflow:hidden">');
T.push('    <div @click="toggle(i)" style="display:flex;align-items:center;gap:10px;padding:12px 16px;cursor:pointer;user-select:none" :style="{background:opened[i]?\'#f8f9fa\':\'white\'}">');
T.push('      <span style="font-size:14px;color:#999;transition:transform 0.2s" :style="{transform:opened[i]?\'rotate(90deg)\':\'rotate(0)\'}">&#9654;</span>');
T.push('      <strong style="font-size:15px;color:#1a4d8f">{{ d.name }}</strong>');
...
T.push('    </div>');
T.push('    <div v-show="opened[i]" style="padding:0 16px 16px">');
// Gauge zones table
...
T.push('    return {');
T.push('      drives: [], sites: [], saving: false, info: "", infoOk: true, opened: {}, siteOpen: {},');
T.push('      defaults: {');
T.push('        CFW900: {');
...
T.push('    toggle: function(i) { this.opened[i] = !this.opened[i]; },');
T.push('    toggleSite: function(s) { this.siteOpen[s] = !this.siteOpen[s]; },');
T.push('    drivesForSite: function(s) { return this.drives.filter(function(d){return d.site===s}); },');
...
T.push('          siteSet[dev.site || "Sin Sitio"] = true;');
T.push('          return { name:dev.name, type:dev.type, site:dev.site||"Sin Sitio", zones:dz };');
T.push('        });');
T.push('        self.sites = Object.keys(siteSet);');
T.push('        self.sites.forEach(function(s){ self.siteOpen[s] = true; });');
